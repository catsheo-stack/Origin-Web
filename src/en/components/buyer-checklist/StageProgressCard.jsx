...
export default function StageProgressCard({ number, name, percentage, completed, total, status, isCurrent, isOpen, onClick }) {
  ...
  return (
    <button
      ...
      onClick={onClick}
      aria-pressed={isOpen}
      aria-label={`Stage ${number}: ${name}, ${percentage}% complete, status ${status}${isCurrent ? ", current stage" : ""}`}
    ...
    >
      ...
      <div className="flex items-center gap-1 mb-2">
        ...
        {isCurrent && (
          <span className="text-[10px] font-medium text-blue-500 ml-1">· Current</span>
        )}
      </div>
