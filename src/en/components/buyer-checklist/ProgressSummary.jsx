    ...
    <div className="bg-white rounded-xl border p-4 sm:p-5 mb-6 shadow-sm">
      ...
      <div className="flex items-center gap-4 mb-3">
        <div className="flex-shrink-0 text-center">
          ...
          <span className="block text-[10px] uppercase tracking-wider text-gray-400 mt-0.5">Complete</span>
        </div>
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-gray-500 mt-1.5">
            <span className="font-medium text-gray-700">{overall.completed}</span> completed
            {" · "}
            <span className="font-medium text-gray-700">{overall.remaining}</span> remaining
            {" · "}
            <span className="text-gray-400">{overall.total} total</span>
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-3 border-t">
        ...
        <div className="flex items-start gap-2">
          ...
          <div className="min-w-0">
            <p className="text-[10px] uppercase tracking-wider text-gray-400">Current Stage</p>
            {currentStage ? (
        ...
        <div className="flex items-start gap-2">
          ...
          <div className="min-w-0">
            <p className="text-[10px] uppercase tracking-wider text-gray-400">Next Action</p>
            {nextAction ? (
              <div className="min-w-0">
              ...
              <p className="text-sm font-medium text-emerald-600">All tasks complete</p>
            )}
          </div>
        </div>
      </div>
      ...
      {lastSaved && (
        <p className="text-[10px] text-gray-400 mt-3 pt-2 border-t">
          Saved {new Date(lastSaved).toLocaleString(undefined, { dateStyle: "short", timeStyle: "short" })}
        </p>
      )}
    </div>
  );
