...
import { PROPERTY_STATUSES, MAX_LENGTHS, validateProperty } from "@/en/utils/buyerJourneyValidation";
  ...
  useEffect(() => {
    if (open) {
      setForm(property || {
        ...
        listingUrl: "", status: "Saved", buyerRating: 0,
        ...
        <div className="space-y-5 py-2">
          <FormSection title="Main Details">
            ...
            <FormField label={`Advertised price (max ${MAX_LENGTHS.advertisedPrice})`} error={errors.advertisedPrice}>
              <Input className="h-9 text-sm" maxLength={MAX_LENGTHS.advertisedPrice} value={form.advertisedPrice || ""} onChange={e => setField("advertisedPrice", e.target.value)} placeholder="e.g. $550,000" />
            </FormField>
            ...
            <FormField label="Land size">
              <Input className="h-9 text-sm" value={form.landSize || ""} onChange={e => setField("landSize", e.target.value)} placeholder="e.g. 500sqm" />
            </FormField>
          </FormSection>
