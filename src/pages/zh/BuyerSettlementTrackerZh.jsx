...
import { SETTLEMENT_MILESTONES_ZH as SETTLEMENT_MILESTONES } from "@/data/tools/settlementTrackerZh";
...
export default function BuyerSettlementTrackerZh() {
  ...
  useEffect(() => {
    document.title = "買方交割進度追蹤器 | Origin Concierge";
    const setMeta = (sel, attr, content) => {
      let el = document.head.querySelector(sel);
      if (!el) { el = document.createElement("meta"); document.head.appendChild(el); }
      el.setAttribute(attr, content);
    };
    setMeta('meta[name="description"]', "content", "使用 Origin Concierge 互動買方交割追蹤器，從簽署合約至領取鑰匙，全程掌握物業交割進度。");
    setMeta('meta[property="og:title"]', "content", "買方交割進度追蹤器 | Origin Concierge");
    setMeta('meta[property="og:description"]', "content", "使用 Origin Concierge 互動買方交割追蹤器，從簽署合約至領取鑰匙，全程掌握物業交割進度。");
    ...
    const schema = [
      {
        ...
        "@type": "BreadcrumbList",
        itemListElement: [
          { "@type": "ListItem", position: 1, name: "首頁", item: "https://originpropertyconcierge.com.au/" },
          { "@type": "ListItem", position: 2, name: "實用工具", item: "https://originpropertyconcierge.com.au/tools" },
          { "@type": "ListItem", position: 3, name: "買方交割進度追蹤器", item: "https://originpropertyconcierge.com.au/tools/buyer-settlement-tracker" },
        ],
      },
      {
        "@context": "https://schema.org",
        "@type": "WebApplication",
        name: "買方交割進度追蹤器",
        description: "從簽署合約至領取鑰匙，追蹤您的物業交割進度。",
  ...
  const handleDownload = () => {
    ...
    toast({ title: "報告已下載", description: "Your 買方交割進度追蹤器 PDF has been saved." });
  };
  ...
  return (
    <>
      <ToolHero
        eyebrow="Origin 實用工具"
        title="買方交割進度追蹤器"
        subtitle="以簡單清晰的流程，追蹤從簽署合約到交割的每個重要里程碑。"
        primaryCta={{ label: "開始追蹤", href: "#details", analyticsKey: "settlement_start" }}
      />

      <SectionWrapper id="details" className="!pt-12 !pb-8 scroll-mt-24">
        <PropertyDetailsForm values={property} onChange={setProperty} onSubmit={handleStart} started={started} onRestart={() => setResetDialog({ open: true, mode: "restart" })} zh />
      </SectionWrapper>
