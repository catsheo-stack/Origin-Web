 ...
 * Tasks marked "不適用" are excluded from totals.
...
const APPLICABLE_STATUSES = ["Not Started", "In Progress", "Waiting", "已完成"];
...
export function getStageStatus(stage, taskStates, propertyType, today) {
  ...
  if (applicable.length === 0) return "已完成";

  const allCompleted = applicable.every(item => item.state.status === "已完成");
  ...
  const needsAttention = visible.some(item => {
    if (item.state.status === "不適用" || item.state.status === "已完成") return false;
...
export function getNextAction(checklistData, taskStates, propertyType, today) {
  ...
  const incomplete = visible.filter(v =>
    v.state.status !== "已完成" && v.state.status !== "不適用"
  );
  ...
  if (currentStage) {
    for (const cat of currentStage.categories) {
      for (const task of cat.tasks) {
        ...
        if (state && state.status !== "已完成" && state.status !== "不適用") {
...
export function getImportantDateStatus(dateRec, today) {
  const t = today || getTodayStart();
  if (dateRec.status === "已完成") return "已完成";
