...
import DashboardTaskPanel from "@/components/buyer-checklist/DashboardTaskPanel";

export default function DashboardSummaryCards({ counts }) {
  const [expanded, setExpanded] = useState(null);

  const cards = [
    { key: "dueSoon", label: "即將到期", icon: CalendarClock, color: "text-blue-500", bg: "bg-blue-50 border-blue-100", data: counts.dueSoon },
    { key: "overdue", label: "已逾期", icon: AlertTriangle, color: "text-red-500", bg: "bg-red-50 border-red-100", data: counts.overdue },
    { key: "waiting", label: "等待中", icon: Clock, color: "text-amber-500", bg: "bg-amber-50 border-amber-100", data: counts.waiting },
    { key: "highRisk", label: "高風險", icon: ShieldAlert, color: "text-red-500", bg: "bg-red-50 border-red-100", data: counts.highRisk },
    { key: "professionalReview", label: "專業覆核", icon: Users, color: "text-indigo-500", bg: "bg-indigo-50 border-indigo-100", data: counts.professionalReview }
  ];
  ...
  return (
    <div className="mb-6">
      <h2 className="text-xs font-semibold uppercase tracking-wider text-gray-500 mb-2.5">需要注意</h2>
          ...
          return (
            <div key={card.key} className={`rounded-lg border ${hasTasks ? card.bg : "bg-gray-50 border-gray-100"} ${isExpanded ? "ring-1 ring-gray-300" : ""}`}>
              <button
                ...
                aria-label={`${card.label}：${card.data.count} 項${hasTasks ? "，按一下展開" : ""}`}
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {card.key === "highRisk" && card.data.hasDealBreaker && (
                    <span className="text-[9px] text-red-600 font-medium">
                      {card.data.dealBreakerCount} 項重大風險
                    </span>
                  )}
                </div>
                {hasTasks && (
