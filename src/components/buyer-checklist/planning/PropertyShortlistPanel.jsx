...
import PropertyCard from "@/components/buyer-checklist/planning/PropertyCard";
import PropertyFormDialog from "@/components/buyer-checklist/planning/PropertyFormDialog";
...
export default function PropertyShortlistPanel({ properties, preferences, selectedPropertyIds, onAdd, onEdit, onDelete, onTogglePriority, onToggleComparison, onToggleReject }) {
  ...
  const filtered = properties.filter(p => {
    ...
    if (filter === "active") return p.status !== "已淘汰" && p.status !== "Purchased";
    if (filter === "rejected") return p.status === "已淘汰";
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          <h2 className="text-lg font-semibold text-gray-900">我的候選物業</h2>
          <p className="text-sm text-gray-500">在最多三項物業上點選「＋比較」，再開啟「比較」分頁並排查看。</p>
        </div>
      ...
      {sorted.length === 0 ? (
        <div className="bg-white rounded-lg border p-8 text-center">
          <p className="text-sm text-gray-400">尚未儲存任何物業。</p>
        </div>
      ) : (
