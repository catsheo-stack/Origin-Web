...
import { DATE_TYPES, DATE_STATUSES, validateImportantDate } from "@/utils/buyerJourneyValidation";
...
export default function ImportantDateFormDialog({ open, dateRecord, properties, onClose, onSave }) {
  const [form, setForm] = useState({
    dateType: "看房", date: "", time: "", propertyId: "", status: "Upcoming", note: ""
  });
  ...
  useEffect(() => {
    if (open) {
      setForm(dateRecord ? {
        dateType: dateRecord.dateType || "看房",
    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-md max-h-[85dvh] overflow-y-auto overscroll-contain">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <FormField label="日期類型" error={errors.dateType}>
          ...
          <FormField label="日期" error={errors.date}>
