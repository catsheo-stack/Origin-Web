    ...
    <div className="flex items-start gap-2 p-2 rounded-md bg-gray-50 border border-gray-100">
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-1 flex-wrap">
          ...
          {item.state.riskLevel === "High" && <span className="text-[10px] text-red-500 font-medium">高風險</span>}
        </div>
      </div>
...
export default function DashboardTaskPanel({ data }) {
  ...
  if (!hasAny) {
    return <div className="p-3 text-xs text-gray-400 italic">此分類暫無項目。</div>;
  }
  ...
  return (
    <div className="p-2 sm:p-3 space-y-3 max-h-64 overflow-y-auto">
      {tasks.length > 0 && (
        <div>
          <p className="text-[10px] font-semibold uppercase tracking-wider text-gray-400 mb-1.5">清單任務</p>
      ...
      {dates.length > 0 && (
        <div>
          <p className="text-[10px] font-semibold uppercase tracking-wider text-gray-400 mb-1.5">重要日期</p>
          <div className="space-y-1.5">
      ...
      {risks.length > 0 && (
        <div>
          <p className="text-[10px] font-semibold uppercase tracking-wider text-gray-400 mb-1.5">已記錄風險</p>
          <div className="space-y-1.5">
