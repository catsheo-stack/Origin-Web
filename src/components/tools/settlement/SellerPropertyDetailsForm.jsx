          ...
          <div className="grid grid-cols-2 gap-2">
            {SELLER_TYPES.map((t) => (
              <button
                key={t}
                type="button"
                onClick={() => onChange({ ...values, sellerType: t })}
                className={`rounded-xl border px-4 py-3 text-sm font-medium transition ${values.sellerType === t ? "border-golden bg-golden/10 text-midnight" : "border-stone bg-parchment/40 text-midnight/60 hover:border-golden/40"}`}
              >
                {t}
              </button>
            ))}
