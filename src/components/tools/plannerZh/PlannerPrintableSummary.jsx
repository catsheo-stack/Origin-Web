    ...
    <div className="printable-summary bg-white rounded-2xl border border-stone/60 overflow-hidden">
      ...
      <div className="p-6 md:p-8 space-y-6">
        ...
        <div>
          ...
          <dl className="grid grid-cols-1 md:grid-cols-2 gap-x-6">
            <Row label="預估購屋預算" value={fmtCurrency(results.estimatedPurchaseBudget)} />
            <Row label="預估貸款金額" value={fmtCurrency(results.loanAmount)} />
            ...
            <Row label="還款佔收入比例" value={fmtPct(results.repaymentToIncomeRatio)} />
            <Row label="Debt to Income Ratio" value={fmtPct(results.debtToIncomeRatio)} />
            <Row label="交割後剩餘現金" value={fmtCurrency(results.cashRemaining)} />
            <Row label="緊急備用金" value={`${results.emergencyBuffer} 個月`} />
          </dl>
        </div>

        {/* 購屋成本明細 */}
        <div>
          <SectionTitle>購屋成本明細</SectionTitle>
          <dl className="grid grid-cols-1 md:grid-cols-2 gap-x-6">
            <Row label="印花稅" value={fmtCurrency(results.buyingCosts.stampDuty)} />
            <Row label="產權過戶服務費" value={fmtCurrency(results.buyingCosts.conveyancingFee)} />
            <Row label="建築驗屋費" value={fmtCurrency(results.buyingCosts.buildingInspection)} />
            <Row label="蟲害檢查費" value={fmtCurrency(results.buyingCosts.pestInspection)} />
            <Row label="政府登記費" value={fmtCurrency(results.buyingCosts.governmentRegistrationFees)} />
            <Row label="產權轉讓費" value={fmtCurrency(results.buyingCosts.transferFees)} />
            <Row label="按揭登記費" value={fmtCurrency(results.buyingCosts.mortgageRegistration)} />
            <Row label="Loan Establishment" value={fmtCurrency(results.buyingCosts.loanEstablishmentCosts)} />
            {results.buyingCosts.lmi > 0 && <Row label="貸款機構按揭保險" value={fmtCurrency(results.buyingCosts.lmi)} />}
        ...
        <div>
          ...
          <dl className="grid grid-cols-1 md:grid-cols-2 gap-x-6">
            ...
            <Row label="Loan-to-Value Ratio" value={fmtPct(results.lvr)} />
            <Row label="交割後剩餘現金" value={fmtCurrency(results.cashRemaining)} />
            <Row label="緊急備用金 (個月)" value={`${results.emergencyBuffer} 個月`} />
          </dl>
        </div>

        {/* 財務舒適度評分 */}
        <div>
          <SectionTitle>財務舒適度評分</SectionTitle>
        ...
        {isFHB && (
          <div>
            <SectionTitle>首次置業者優惠</SectionTitle>
            <dl className="grid grid-cols-1 md:grid-cols-2 gap-x-6 mb-3">
              <Row label="預估印花稅節省" value={fmtCurrency(fhbSavings)} />
              <Row label="State" value={inputs.state} />
            </dl>
            <ul className="space-y-1.5">
              <li className="text-sm text-midnight/70 leading-relaxed">&bull; 首次置業補助 (FHOG) — may be available for new homes.</li>
              <li className="text-sm text-midnight/70 leading-relaxed">&bull; 首次置業擔保計畫 — may allow a 5% deposit without LMI.</li>
              <li className="text-sm text-midnight/70 leading-relaxed">&bull; 首次置業退休金儲蓄計畫 (FHSS) — withdraw voluntary super contributions toward deposit.</li>
        ...
        {isInvestor && investorMetrics && (
          <div>
            ...
            <dl className="grid grid-cols-1 md:grid-cols-2 gap-x-6">
              ...
              <Row label="預估持有成本" value={fmtCurrency(investorMetrics.estimatedHoldingCosts)} />
        ...
        {/* 實際狀況檢視 */}
        <div>
          <SectionTitle>實際狀況檢視</SectionTitle>
          <ul className="space-y-1.5">
