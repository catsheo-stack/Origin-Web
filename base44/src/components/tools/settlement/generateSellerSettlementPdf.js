...
export function generateSellerSettlementPdf({ property, milestones, currentStage, nextAction, generatedDate }) {
  ...
  doc.text("Seller Settlement Tracker", margin, 54);
  doc.setFontSize(9);
  doc.setTextColor(...PARCHMENT);
  ...
  row("Seller Type", property.sellerType);
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.setTextColor(...MUTED);
    doc.setFont("helvetica", "normal");
    doc.text("Origin Property Concierge  |  Seller Settlement Tracker", margin, pageH - 26);
    doc.text(`Page ${i} of ${pageCount}`, pageW - margin, pageH - 26, { align: "right" });
  }

  doc.save("Seller-Settlement-Tracker.pdf");
}